(import ./location-with-unreadable :as lwu)
(import ./jipper :as j)
(import ./print :as pr)
...
(defn rewrite-test-zloc
  [test-zloc]
  (deprintf "test:")
  (deprintf (lwu/gen (j/node test-zloc)))
    ...
    (let [things (find-things test-zloc)]
      ...
      (when (get names blank-char-str)
        ...
        (set chosen-thing-zloc
           (rnd/choose things))
      (deprintf "chosen: %s" (lwu/gen (j/node chosen-thing-zloc))))
    #
    (do
